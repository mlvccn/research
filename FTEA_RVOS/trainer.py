...
from add.partial_load import load_checkpoint_free
import json
import pdb 
...
class Trainer:
        ...
        for self.epoch in tqdm(range(self.epoch, self.total_epochs), disable=not self.is_main_process):
            ...
            for batch_dict in tqdm(self.data_loader_train, disable=not utils.is_main_process()):
                ...
                with amp.autocast(enabled=self.config.enable_amp):
                    ...
                    kernel_loss = torch.mean(outputs['kernel_loss'])
                    losses = losses + kernel_loss 
    ...
    def save_masks(self, coco_eval, predictions_with_mask):
        ...
        mask_json_dir = path.join(annotations_dir, "mask.json")
        tmp = json.dumps(predictions_with_mask,cls=MyEncoder,indent=4)
        with open(mask_json_dir, 'w') as f:
            json.dump(tmp, f)
        proposal_num = len(predictions_with_mask) // len(coco_eval.ious.keys())
        for i, proposal in enumerate(predictions_with_mask): # proposals number x validation frame number
            image_id = proposal['image_id'] # v_-4KMQsUaSAQ_f_28_i_0
            mask = proposal['segmentation'] # 1,320,568
            if i% proposal_num ==0:
                iou_scores = coco_eval.ious[(image_id, -1)]
            if iou_scores[i%proposal_num] > 0.5:
                mask_save_dir = path.join(annotations_dir, "{}_{}.png".format(image_id, i))
                mask = mask.squeeze(0) * 255
    ...
    def evaluate_a2d_sentences(self):
        ...
        for batch_dict in tqdm(self.data_loader_val, disable=not self.is_main_process):
            ...
            for p, image_id in zip(processed_outputs, image_ids):
                for s, m, msk in zip(p['scores'], p['rle_masks'], p['masks']):
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        if 'epoch' not in checkpoint:
            ...
            model_without_ddp = load_checkpoint_free(pretrained_dict=checkpoint, model=model_without_ddp)
        ...
        if self.total_epochs < checkpoint['total_epochs']:
        ...
        try:
            ...
            model_without_ddp = load_checkpoint_free(checkpoint['model_state_dict'], model_without_ddp)
