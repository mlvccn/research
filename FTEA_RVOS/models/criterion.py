...
from add.loss import triplet_cycle_loss, triplet_cycyle_loss_v3, triplet_cycyle_loss_v4
...
class SetCriterion(nn.Module):
    ...
    def compute_trip_cycle(self, feats, targets, margin=0.4):
        targets = [frame_t_target for step_t in targets for frame_t_target in step_t]

        masks = [t["masks"] for t in targets]
        target_masks, valid = nested_tensor_from_tensor_list(masks).decompose()
        target_masks = target_masks.to(feats.device)
        b,o, h,w = target_masks.shape
        gt = target_masks.view(b*o, h,w)
        # t,c,h,w ; t*b*o,h,w
        # gt = target_masks  # b, o, h, w
        # when A2D, only one frame annotated in a vide clip
        # feats shape is b c h w
        # loss = triplet_cycle_loss(feats.unsqueeze(1), gt, margin=margin)
        loss = triplet_cycyle_loss_v4(feats.unsqueeze(1), gt, margin=margin)
        return loss
        # target_masks = target_masks[tgt_idx]