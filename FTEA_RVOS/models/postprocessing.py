...
from add.vis_utils import tensor_to_im
...
class A2DSentencesPostProcess(nn.Module):
    ...
    def forward(self, outputs, resized_padded_sample_size, resized_sample_sizes, orig_sample_sizes, images=None,valid_indices=None):
        ...
        """
        cur_frames = images.tensors[valid_indices].squeeze(0) # [batch, 3, h, w]
        pred_is_referred = outputs['pred_is_referred']
        prob = F.softmax(pred_is_referred, dim=-1)
        scores = prob[..., 0]
        pred_trajectory_indices = torch.argmax(scores, dim=-1)
        ...
        batch_idx = 0
        for f_pred_masks, resized_size, orig_size, cur_frame in zip(pred_masks, resized_sample_sizes, orig_sample_sizes, cur_frames):
