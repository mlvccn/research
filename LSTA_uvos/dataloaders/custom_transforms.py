import os
import random
from utils.checkpoint import save_network
import cv2
import numpy as np
import torch
...
#                 A.ColorJitter(),
#                 A.RandomBrightness(),
#                 A.GridDropout(ratio=0.3, unit_size_min=None, unit_size_max=None, holes_number_x=3, holes_number_y=3, shift_x=0, shift_y=0, random_offset=True, fill_value=0, mask_fill_value=None, always_apply=False, p=0.5),
#                 A.imgaug.transforms.IAAAffine(scale=1.0, translate_percent=None, translate_px=(-15, 15), rotate=(-30, 30), shear=(-15, 15), order=1, cval=0, mode='reflect', always_apply=False, p=0.5),
...
#         ref_label = sample['ref_label']
#         augmented = self.aug(image=ref_img, mask=ref_label)
#         sample['ref_img'] = augmented['image']
#         sample['ref_label'] = augmented['mask']


#         prev_img = sample['prev_img']
#         prev_label = sample['prev_label']
#         augmented = self.aug(image=prev_img, mask=prev_label)
#         sample['prev_img'] = augmented['image']
#         sample['prev_label'] = augmented['mask']
        
#         curr_img = sample['curr_img']
#         curr_label = sample['curr_label']
#         for i in range(len(curr_img)):
#             img = curr_img[i]
#             label = curr_label[i]
#             augmented = self.aug(image=img, mask=label)
#             sample['curr_img'][i] = augmented['image']
#             sample['curr_label'][i] = augmented['mask']
...
#         ref_img = sample['ref_img']
#         ref_label = sample['ref_label']
#         ref_img, ref_label = self.affinity(image = ref_img,segmentation_maps = ref_label[np.newaxis,:,:,np.newaxis])
#         sample['ref_img'] = ref_img
#         sample['ref_label'] = ref_label[0,:,:,0]

#         prev_img = sample['prev_img']
#         prev_label = sample['prev_label']
#         prev_img, prev_label = self.affinity(image = prev_img,segmentation_maps = prev_label[np.newaxis,:,:,np.newaxis])
#         sample['prev_img'] = prev_img
#         sample['prev_label'] = prev_label[0,:,:,0]
...
#             img = curr_img[i]
#             label = curr_label[i]
#             img, label = self.affinity(image = img,segmentation_maps = label[np.newaxis,:,:,np.newaxis])
#             sample['curr_img'][i] = img
#             sample['curr_label'][i] = label[0,:,:,0]
