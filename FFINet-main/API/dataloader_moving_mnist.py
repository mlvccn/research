...
def load_fixed_set(root):
    ...
    # dataset = dataset.transpose(1,0,2,3,4)
    dataset = dataset[..., np.newaxis]
    return dataset

def load_fixed_mask(root):
    # Load the fixed dataset
    filename = 'moving_mnist/mask.npy'
    path = os.path.join(root, filename)
    fixed_mask = np.load(path)
    return fixed_mask

class MovingMNIST(data.Dataset):
    def __init__(self, root, is_train=True, n_frames_input=10, n_frames_output=10, num_objects=[2], with_mask=0,
                 transform=None):
        ...
        if is_train:
            ...
            if num_objects[0] != 2:
                ...
                self.mask = load_fixed_mask(root)
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train or self.num_objects[0] != 2:
            ...
            if self.with_mask:
                all_masks = create_random_shape_with_random_motion(
                    self.n_frames_input, imageHeight=self.image_size_, imageWidth=self.image_size_
                )
                all_masks = np.stack([np.expand_dims(x, 2) for x in all_masks], axis=0)/ 255.0  
        else:
            images = self.dataset[:, idx, ...]
            if self.with_mask:
                all_masks = self.mask[:,idx,...]
        ...
        if self.with_mask:
            return input, output, all_masks.transpose(0,3,1,2)
        else:
            return input, output
...
def load_data(
    ...
    train_set = MovingMNIST(root=data_root, is_train=True,
                            n_frames_input=10, n_frames_output=10, num_objects=[2], with_mask=mask)
